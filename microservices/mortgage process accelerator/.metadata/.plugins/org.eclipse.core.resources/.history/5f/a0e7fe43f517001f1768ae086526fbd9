...
public class LoanProductServiceImpl implements ILoanProductService {
	...
	public void create(LoanProductDto loanProductDto) {
		...
		if (loanProdObj.getStatus()) {
			...
			criteriaStatus = addUnderWritingCriteria(loanProdObj.getProductId(), loanProd.getUnderwritingCriteria());
		...
		if (loanProdObj.getEscrowRequired()) {
			 ...
			 escrowStatus = addEscrowRequirements(loanProdObj.getProductId(), loanProd.getEscrowRequirements());
		...
		if(escrowStatus && criteriaStatus) {
			
			LoanReqDto loanReqDto = new LoanReqDto(loanProdObj.getProductId(), loanProd.getEscrowRequirements());
			
			
			escrowClient.save(loanReqDto);
			
			criteriaClient.addCriteriaToLoanProd(loanProdObj.getProductId(),loanProd.getUnderwritingCriteria());
	 ...
	 *                                   are not found.
	 */
	public Boolean addUnderWritingCriteria(Integer productId, List<String> underwritingCriteria) {
	...
	Boolean  addEscrowRequirements(Integer productId, List<String> escrowRequirements) {
	...
	public void addEscrowRequirementsToLoanProduct(Integer productId, List<String> escrowRequirements) {
		...
		Boolean escrowStatus = addEscrowRequirements(productId, escrowRequirements);
	...
	public void addUnderWritingCriteriaToLoanProduct(Integer productId, List<String> underwritingCriteria) {
		...
		Boolean criteriaStatus = addUnderWritingCriteria(productId, underwritingCriteria);
		
		if(criteriaStatus) {
			criteriaClient.addCriteriaToLoanProd(productId,underwritingCriteria);
		}
	...
	public void updateLoanProductStatus(Integer productId) {
		...
		if (status) {
			...
			List<UnderwritingCriteriaDto> criteriaDtoList = criteriaClient.fetchByLoanId(productId).getBody();
			
			if(criteriaDtoList.isEmpty() ){
				
				throw new ResourceNotFoundException(AppConstants.LOAN_STATUS_ERROR);
			}
	...
	public void updateLoanProduct(Integer productId, LoanProductDto loanProductDto) {
		...
		if (loanProduct.getProductName().equalsIgnoreCase(loanProductDto.getProductName())) {
			...
			loanProductObj.setPrepayPenalty(loanProduct.getPrepayPenalty());
			loanProductObj.setPropertyRestrictionExists(loanProduct.getPropertyRestrictionExists());
			loanProductObj.setEscrowRequired(loanProduct.getEscrowRequired());
			loanProductObj.setStatus(loanProduct.getStatus());
	...
	public List<String> checkLoanProductRestrictions(List<String> rsrtTypes) {
	...
	public List<PropertyRestriction> getExistsPropertyRestrictions(List<String> rsrtTypes) {
	...
	public void deleteEscrowRequirements(Integer productId, List<String> escrowRequirements) {
